...
const seed = async () => {
  ...
  if (!hotel) {
    hotel = await Hotel.create({ name: "Test Hotel", email: "test@hotelos.com" });
  }

  let room = await Room.findOne({ roomNumber: "204", hotelId: hotel._id });
  if (!room) {
    room = await Room.create({ roomNumber: "204", type: "Deluxe", hotelId: hotel._id, status: "OCCUPIED" });
  }
  ...
  if (existingItems === 0) {
    await FoodItem.insertMany([
      { name: "Butter Chicken", price: 680, category: "Main Course", hotelId: hotel._id },
      { name: "Veg Biryani", price: 360, category: "Main Course", hotelId: hotel._id },
      { name: "Cold Coffee", price: 180, category: "Beverages", hotelId: hotel._id },
    ]);
