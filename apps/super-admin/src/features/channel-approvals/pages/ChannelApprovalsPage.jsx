...
import {
  useChannelApprovals,
  useVerifyChannelApproval,
} from "../hooks/useChannelApprovals.js";
...
export default function ChannelApprovals() {
  ...
  const verifyApprovalMutation = useVerifyChannelApproval();
  const error = queryError
    ? queryError.message || "Failed to load channel approvals"
    : "";
  ...
  const handleVerify = async (id, { skipRefresh = false } = {}) => {
    setToast(null);
    try {
      const result = await verifyApprovalMutation.mutateAsync(id);
