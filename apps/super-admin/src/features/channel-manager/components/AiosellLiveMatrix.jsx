...
import {
  ...
  useUpdateLiveRates,
  useLiveInventory,
  useUpdateLiveInventory,
  useUpdateRateRestrictions,
  useUpdateInventoryRestrictions,
  useMarkChannelNoShow,
} from "../hooks/useChannelManager.js";

/**
 * Formats an ISO date string into day and weekday headers.
 * @param {string} dateStr
 * @returns {{ day: string, weekday: string }}
 */
...
/**
 * Adds days to an ISO date string and returns formatted date (YYYY-MM-DD).
 * @param {string} dateStr
 * @param {number} count
 * @returns {string}
 */
...
/**
 * Live rates, inventory, and restrictions matrix for Aiosell integration.
 * @param {object} props
 * @param {string} props.hotelId
 * @param {string} props.hotelName
 * @returns {JSX.Element}
 */
export default function AiosellLiveMatrix({ hotelId, hotelName }) {
  ...
  } = useLiveRates({ hotelId, startDate, endDate });
  ...
  } = useLiveInventory({ hotelId, startDate, endDate });

  const updateLiveRatesMut = useUpdateLiveRates();
  const updateLiveInventoryMut = useUpdateLiveInventory();
  const updateRateRestrictionsMut = useUpdateRateRestrictions();
  const updateInventoryRestrictionsMut = useUpdateInventoryRestrictions();
  const markChannelNoShowMut = useMarkChannelNoShow();

  const loading = ratesLoading || invLoading;
  const error =
    ratesError || invError
      ? ratesError?.message ||
        invError?.message ||
        "Failed to load live data from Aiosell"
      : "";

  const refetchAll = async () => {
    await Promise.all([refetchRates(), refetchInventory()]);
  };
  ...
  const handleSaveCell = async (e) => {
    ...
    try {
      if (cellEditModal.type === "rate") {
        ...
        await updateLiveRatesMut.mutateAsync({
          hotelId,
          updates: [
            {
        ...
        await updateLiveInventoryMut.mutateAsync({
          hotelId,
          updates: [
            {
              startDate: cellEditModal.date,
  ...
  const handleRestrictionsSubmit = async (e) => {
    ...
    try {
      ...
      if (restScope === "rate" || restScope === "both") {
        ...
        calls.push(
          updateRateRestrictionsMut.mutateAsync({
            hotelId,
            updates: [
      ...
      if (restScope === "inventory" || restScope === "both") {
        ...
        calls.push(
          updateInventoryRestrictionsMut.mutateAsync({
            hotelId,
            updates: [
  ...
  const handleNoShowSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await markChannelNoShowMut.mutateAsync({
        hotelId,
        bookingId: noShowBookingId.trim(),
      });
