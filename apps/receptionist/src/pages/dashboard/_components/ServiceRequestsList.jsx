...
export default function ServiceRequestsList({
  ...
  return (
    <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-xs">
      <h2 className="text-brand-900 mb-3 flex items-center gap-2 font-bold">
          ...
          .map((req) => (
            <div key={req.id} className="rounded-xl border border-gray-100 p-3">
                  ...
                  {req.items.length > 0 && (
                    <div className="mt-1.5 flex flex-wrap gap-1">
                      {req.items.map((item) => (
                        <span
                          key={item}
                          className="rounded-md bg-amber-100 px-1.5 py-0.5 text-[9px] font-semibold text-amber-700"
                        >
                          {item}
                        </span>
                      ))}
              ...
              <div className="mt-2 flex items-center gap-2">
                <span
                  className={`rounded-full px-2 py-0.5 text-[9px] font-semibold ${req.status === "requested" ? "bg-amber-100 text-amber-600" : req.status === "acknowledged" ? "bg-blue-100 text-blue-600" : "bg-green-100 text-green-600"}`}
                >
                ...
                {req.status === "requested" && (
                  <button
                    ...
                    className="bg-brand-900 hover:bg-brand-800 rounded-full px-2 py-0.5 text-[9px] text-white transition-colors"
                  >
                    Acknowledge
                  </button>
                )}
