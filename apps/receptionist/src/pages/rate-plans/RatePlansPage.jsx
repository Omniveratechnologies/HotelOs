...
import RatePlanModal from "./RatePlanModal.jsx";
import AiosellRatesMatrix from "./AiosellRatesMatrix.jsx";
import {
  getRatePlans,
  createRatePlan,
  updateRatePlan,
...
export default function RatePlansPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!cancelled) setRatePlans(data);
      } catch (err) {
        console.error("Failed to load rate plans:", err);

        if (!cancelled) setError(err.message || "Failed to load rate plans");
      } finally {
  ...
  const handleDelete = async (plan) => {
    ...
    try {
      ...
      setRatePlans((prev) => prev.filter((item) => item.id !== plan.id));
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              {loading ? "Loading..." : `${ratePlans.length} rate plans`}
            </p>
          </div>
        </div>
      ...
      {showModal && (
        <RatePlanModal
          ...
          onSaved={async (payload) => {
            const saved = editing
              ? await updateRatePlan(editing.id, payload)
