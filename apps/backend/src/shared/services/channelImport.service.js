...
export async function syncChannelFromAiosell(hotelId) {
  ...
  // --------------------------------------------------------------
  // Record every pulled RATE_PLAN and ROOM_TYPE code as an
  // ALREADY-APPROVED approval so the "under review" pipeline starts
  // green for codes that exist in Aiosell (they are real, so they are
  // approved by definition). ROOM requests are per-room now, so rooms
  // need no code-level approval.
  // --------------------------------------------------------------
  const now = new Date();
  const approvalDocs = [];

  for (const rp of ratePlansToCreate) {
    approvalDocs.push({
      hotelId,
      kind: "RATE_PLAN",
  ...
  // Reconciliation replaces multiple collections. Keep that destructive work
  // atomic so an insertion failure cannot leave the hotel half-imported.
  let reconciliation;
  const session = await mongoose.startSession();

  try {
    await session.withTransaction(async () => {
      const [roomDelete, planDelete, roomTypeDelete, approvalDelete] =
        await Promise.all([
          Room.deleteMany({ hotelId }, { session }),
          RatePlan.deleteMany({ hotelId }, { session }),
          RoomType.deleteMany({ hotelId }, { session }),
          ChannelApproval.deleteMany({ hotelId }, { session }),
        ]);

      const createdRoomTypes = await RoomType.insertMany(roomTypesToCreate, {
        session,
      });
      const createdRooms = await Room.insertMany(roomsToCreate, { session });
      const createdPlans =
        ratePlansToCreate.length > 0
          ? await RatePlan.insertMany(ratePlansToCreate, { session })
          : [];
      const createdApprovals =
        approvalDocs.length > 0
          ? await ChannelApproval.insertMany(approvalDocs, { session })
          : [];
      ...
      reconciliation = {
        ...
        approvals: {
          deleted: approvalDelete.deletedCount ?? 0,
  ...
  return {
    ...
    data: {
      ...
      roomTypes: reconciliation.roomTypes,
      ratePlans: reconciliation.ratePlans,
      approvals: reconciliation.approvals,
